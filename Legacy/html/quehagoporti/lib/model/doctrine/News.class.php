...
class News extends BaseNews
{
	...
	{	 
	  ...
	  try
	  {
		...
		if( file_exists( $dir . '/news/'.$this->getPicture() ) )
		{
		// Normal Image
		$image = new SimpleImage();
		$image->load( $dir . '/news/'.$this->getPicture() );
		...
		if( $image->getHeight() > 0 && $image->getWidth() > 0 && ( $image->getHeight() > 620 || $image->getWidth() > 620 ) )
		{
			...
			$image->save( $dir . '/news/'.$this->getPicture() );
		}
		// End Normal Image
		
		// Thumbnail Image
		$thumbnail = new SimpleImage();
		$thumbnail->load( $dir . '/news/'.$this->getPicture() );
		...
		if( $thumbnail->getHeight() > 0 && $thumbnail->getWidth() > 0 && ( $thumbnail->getHeight() > 100 || $thumbnail->getWidth() > 100 ) )
		{
			...
			$thumbnail->save( $dir.'/news_thumbnail/'.$this->getPicture() );
		}
		// End Thumbnail Image
		}
		$conn->commit();
