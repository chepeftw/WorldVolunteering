...
<form action="<?php echo url_for('association_survey/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
	...
	<table>
      <tr>
        <td id="save" colspan="2" >
		  <input type="hidden" value="<?php echo $sf_request->getRemoteAddress() ?>" id="association_survey_ip_address" name="association_survey[ip_address]" />
